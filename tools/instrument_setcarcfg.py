...
Installs a logging trampoline on the SECOND SetCarCfg call site
(0x004AE080, found via static xref — the pnach build-13 payload owns the
first site 0x004AE100). Each call records a0-a3 + caller RA into a ring
buffer at 0x00720700 that Python can read.
...
Usage:
  python tools/instrument_setcarcfg.py install    # write trampoline + patch JAL
  python tools/instrument_setcarcfg.py status     # dump ring + decode vehNames
  python tools/instrument_setcarcfg.py restore    # unpatch
"""
...
CALL_SITE = 0x004AE080            # unpatched JAL 0x005D2380 (static xref)
ORIG_INSTR = 0x0C1748E0           # JAL 0x005D2380
ORIG_TARGET = 0x005D2380
TRAMP_ADDR = 0x00720800           # free payload area past the build-13 driver
...
def build_trampoline() -> bytes:
    ...
    # call original SetCarCfg
    asm.lui(R.T9, (ORIG_TARGET >> 16) & 0xFFFF)
    asm.ori(R.T9, R.T9, ORIG_TARGET & 0xFFFF)
    asm.jalr(R.T9)
    asm.nop()
...
def cmd_install():
    ...
    print(f"installed: {len(code)} bytes at 0x{TRAMP_ADDR:08X}, "
          f"JAL patched at 0x{CALL_SITE:08X}")
    game.close()


def cmd_restore():
    game = connect()
    game.write_u32(CALL_SITE, ORIG_INSTR)
    print("restored original JAL")
    game.close()
